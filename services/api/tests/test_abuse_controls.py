...
@pytest.mark.asyncio
async def test_booking_and_ai_routes_are_limited(monkeypatch: pytest.MonkeyPatch) -> None:
    _override_rules(
        monkeypatch,
        **{
            "ai-generate": Rule("ai-generate", None, Allowance(1, 600), "test"),
            "booking": Rule("booking", None, Allowance(1, 600), "test"),
        },
    )
    async with _client() as member:
        await _register(member)
        ...
        cancel = f"/api/v1/checkout/{uuid4()}/cancel"
        body = {"reason": "Plans changed this week"}
        assert (await member.post(cancel, json=body)).status_code == 404
        assert (await member.post(cancel, json=body)).status_code == 429
...
@pytest.fixture
async def business(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> AsyncGenerator[dict[str, Any], None]:
    monkeypatch.setattr(settings, "private_storage_dir", str(tmp_path))
    async with _client() as owner:
        await _register(owner, "uploader")
        org = (await owner.post("/api/v1/portal/organizations", json={"name": f"Upload {uuid4().hex[:6]}"})).json()
        venue = await owner.post(
            f"/api/v1/portal/organizations/{org['id']}/venues",
            json={"name": "Hamra", "address": "Hamra", "lng": BEIRUT_LNG, "lat": BEIRUT_LAT},
        )
        experience = await owner.post(
            ...
            json={
                ...
                "suitability": ["groups"],
                "weather": ["all-weather"],
                "price": {"currency": "USD", "price_type": "fixed", "unit": "person", "amount_minor": 1000},
                "policy": {"cancellation_rules": {"hours": 24}, "terms_text": "Cancel 24h before."},
            },
        )
        assert experience.status_code == 200, experience.text
        yield {"client": owner, "org": org["id"], "experience": experience.json()["id"], "root": tmp_path}
...
@pytest.mark.asyncio
async def test_listing_images_go_to_imagekit(business: dict[str, Any], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "imagekit_api_key", "private_test_key")
    monkeypatch.setattr(settings, "imagekit_url", "https://ik.imagekit.io/mshwar")
    fake = FakeImageKit()
    imagekit.set_client(fake)
    try:
        response = await business["client"].post(
            f"/api/v1/portal/organizations/{business['org']}/files", json=_image_upload(business)
        )
    finally:
        imagekit.set_client(None)
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["provider"] == "imagekit"
    assert body["object_key"] == fake.paths[0]
    assert body["preview_url"] == f"https://ik.imagekit.io/mshwar/{fake.paths[0]}"
    assert body["media"]["moderation"] == "pending"
    upload = fake.requests[0]
    assert upload.url.host == "upload.imagekit.io"
    assert upload.headers["authorization"] == "Basic " + base64.b64encode(b"private_test_key:").decode()
    form = upload.content.decode("latin-1")
    assert f"/listings/{business['org']}/{business['experience']}" in form
    assert 'name="isPrivateFile"' in form and "hero.jpg" not in form  # random file name
    assert _files(business["root"]) == []  # nothing processed or kept in the API process


@pytest.mark.asyncio
async def test_imagekit_failure_stores_nothing(business: dict[str, Any], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "imagekit_api_key", "private_test_key")
    monkeypatch.setattr(settings, "imagekit_url", "https://ik.imagekit.io/mshwar")
    imagekit.set_client(FakeImageKit(fail=True))
    try:
        response = await business["client"].post(
            f"/api/v1/portal/organizations/{business['org']}/files", json=_image_upload(business)
        )
    ...
    async with TestingSessionLocal() as session:
        count = (
            await session.execute(
                text("SELECT count(*) FROM app.media WHERE experience_id = :id"), {"id": business["experience"]}
            )
        ).scalar_one()
    assert count == 0
...
@pytest.mark.asyncio
async def test_verification_documents_never_leave_private_storage(
    business: dict[str, Any], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(settings, "imagekit_api_key", "private_test_key")
    monkeypatch.setattr(settings, "imagekit_url", "https://ik.imagekit.io/mshwar")
    fake = FakeImageKit()
    ...
    try:
        response = await business["client"].post(
            f"/api/v1/portal/organizations/{business['org']}/files",
            json={
    ...
    link = response.json()["signed"]["url"]
    download = await business["client"].get(link)
...
@pytest.mark.asyncio
async def test_signed_links_expire(business: dict[str, Any]) -> None:
    upload = await business["client"].post(
        f"/api/v1/portal/organizations/{business['org']}/files", json=_image_upload(business)
    )
    assert upload.status_code == 200, upload.text
    expired = sign_object_url(upload.json()["object_key"], ttl_seconds=-1)
    assert (await business["client"].get(expired["url"])).status_code == 404
    tampered = upload.json()["preview_url"][:-4] + "abcd"
    assert (await business["client"].get(tampered)).status_code == 404


@pytest.mark.asyncio
async def test_rejected_content_is_never_stored(business: dict[str, Any]) -> None:
    response = await business["client"].post(
        f"/api/v1/portal/organizations/{business['org']}/files",
        json=_image_upload(business, tiny_jpeg() + b"<script>x</script>"),
    )
    assert response.status_code == 415
    assert _files(business["root"]) == []


@pytest.mark.asyncio
async def test_organisation_storage_quota(business: dict[str, Any], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "upload_org_quota_bytes", len(tiny_jpeg()) + 10)
    url = f"/api/v1/portal/organizations/{business['org']}/files"
    assert (await business["client"].post(url, json=_image_upload(business))).status_code == 200
    over = await business["client"].post(url, json=_image_upload(business))
    assert over.status_code == 413
    assert over.json()["detail"] == "storage quota exceeded"
    assert len(_files(business["root"])) == 1


@pytest.mark.asyncio
async def test_images_per_listing_are_capped(business: dict[str, Any], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "max_images_per_experience", 2)
    url = f"/api/v1/portal/organizations/{business['org']}/files"
    statuses = [(await business["client"].post(url, json=_image_upload(business))).status_code for _ in range(3)]
    assert statuses == [200, 200, 413]


@pytest.mark.asyncio
async def test_uploads_are_rate_limited_per_organisation(
    business: dict[str, Any], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(settings, "upload_org_hourly_limit", 2)
    url = f"/api/v1/portal/organizations/{business['org']}/files"
    statuses = [(await business["client"].post(url, json=_image_upload(business))).status_code for _ in range(3)]
    assert statuses == [200, 200, 429]
...
@pytest.mark.asyncio
async def test_outsiders_cannot_upload_or_count_against_the_limit(
    business: dict[str, Any], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(settings, "upload_org_hourly_limit", 1)
    ...
    allowed = await business["client"].post(
        f"/api/v1/portal/organizations/{business['org']}/files", json=_image_upload(business)
    )
