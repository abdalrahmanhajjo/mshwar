...
async def test_hub_lists_require_a_session() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as fresh:
        ...
        assert (await fresh.get("/api/v1/bookings")).status_code == 401
        assert (await fresh.get("/api/v1/notifications")).status_code == 401
...
@pytest.mark.asyncio
async def test_bookings_cancel_is_explicit_and_cross_user_blocked(api: AsyncClient) -> None:
    await _register_verified(api, _email("booker"))
    ...
    assert listed.json()["total"] == 1
    assert listed.json()["items"][0]["id"] == booking_id

    missing_reason = await api.post(f"/api/v1/bookings/{booking_id}/cancel", json={"reason": ""})
    assert missing_reason.status_code == 422

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as other:
        await _register_verified(other, _email("stranger"))
        assert (await other.get("/api/v1/bookings")).json()["items"] == []
    ...
    deleted = await api.delete(f"/api/v1/bookings/{booking_id}")
    ...
    assert cancelled.json()["reason"] == "Dates no longer work"

    still_there = await api.get("/api/v1/bookings")
    assert still_there.json()["total"] == 1
    assert still_there.json()["items"][0]["status"] == "cancelled"
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as other:
        await _register(other, _email("reader"))
        assert (await other.get("/api/v1/notifications")).json()["items"] == []
        steal_note = await other.post(f"/api/v1/notifications/{note['id']}/read")
        assert steal_note.status_code == 404

    read = await api.post(f"/api/v1/notifications/{note['id']}/read")
