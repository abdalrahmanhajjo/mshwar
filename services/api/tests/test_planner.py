...
def test_partial_replan_keeps_locked_and_refuses_silent_worse_plan() -> None:
    ...
    stops = [
        ...
        OptimizeStop(id="cafe", lat=33.89, lng=35.47, label="Cafe", duration_minutes=45, estimated_minor=1500),
    ]
    indoor = OptimizeStop(
        id="museum",
        lat=33.895,
    ...
    assert "hike" in result.diff.removed
    assert "museum" in result.diff.added
    assert result.after is not None
    assert result.after.ordered[0].id == "lunch"

    impossible = replan_affected(
        *BEIRUT,
        stops,
        ["hike"],
        [
            OptimizeStop(
                ...
                window_start=start + timedelta(hours=20),
                window_end=start + timedelta(hours=21),
            )
        ],
        start,
    ...
    assert impossible.applied is False
    assert impossible.feasible is False
    assert "unchanged" in impossible.message.lower() or "feasible" in impossible.message.lower()
    assert impossible.after is None
    assert impossible.bookings_mutated is False

    none_affected = replan_affected(*BEIRUT, stops, [], [indoor], start, end, booking_statuses={"bk-1": "confirmed"})
    assert none_affected.applied is False
    assert "No weather-affected" in none_affected.message
    ...
    stuck = replan_affected(*BEIRUT, locked_only, ["hike"], [], start, end)
    assert stuck.applied is False
    assert stuck.feasible is False
