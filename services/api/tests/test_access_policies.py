...
PUBLIC_ROUTES = {
    ...
    "GET /api/v1/checkout/slots/{slug}",
    "POST /api/v1/checkout/quote",
    "GET /api/v1/locations/areas",
    "GET /api/v1/portal/geo/lebanon",
    "GET /api/v1/portal/taxonomy",
    "GET /api/v1/profile/vocabularies",
    "GET /api/v1/privacy/policies",
    "GET /api/v1/reviews",
    "GET /api/v1/reviews/aggregates",
}
TOKEN_ROUTES = {
...
@pytest.mark.asyncio
async def test_token_routes_reject_unknown_tokens_without_detail(anonymous: AsyncClient) -> None:
    for method, path in sorted(key.split(" ", 1) for key in TOKEN_ROUTES):
        body = {"display_name": "Guest"} if path.startswith("/api/v1/groups/join") and method == "POST" else None
        response = await anonymous.request(method, _concrete(path), json=body)
        assert response.status_code in {400, 404}, f"{method} {path} -> {response.status_code} {response.text}"


@pytest.mark.asyncio
async def test_token_routes_are_rate_limited(anonymous: AsyncClient) -> None:
    statuses = [(await anonymous.get(f"/api/v1/notifications/unsubscribe/{'x' * 40}")).status_code for _ in range(31)]
