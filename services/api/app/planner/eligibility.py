...
def _as_time(value: Any) -> time | None:
    ...
    if len(text) >= 5 and text[2] == ":":
        return time(int(text[0:2]), int(text[3:5]))
    return None
...
def evaluate_candidate(
    ...
    *,
    ...
    if from_lat is not None and from_lng is not None:
        minutes, _distance = travel_minutes(from_lat, from_lng, candidate.lat, candidate.lng)
        arrival = start if at is None else at
        travel_end = (constraints.window_start or start) + timedelta(minutes=minutes)
        if at is None:
            arrival = travel_end
