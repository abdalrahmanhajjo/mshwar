...
/**
 * What a place costs, and how to put it into a day.
 *
 * Mshwar plans days; it does not hold reservations. The party size is here
 * because it changes the estimate, not because anything is being booked.
 */
export function PlanPanel({ experience }: { experience: Experience }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-2">
        <Label htmlFor="plan-guests">{copy.guests}</Label>
        <NativeSelect id="plan-guests" value={guests} onChange={(event) => setGuests(Number(event.target.value) || 1)}>
      ...
      <div className="grid gap-3">
        <Button asChild size="lg" className="w-full">
          <LocaleLink href={`/plan?add=${experience.slug}`}>
            {copy.addToDayPlan}
