...
import { CATEGORIES, type ExperienceCategory } from "@/lib/catalog";
...
export function CategoryPills({ active, variant = "chips" }: { active?: string; variant?: "chips" | "icons" }) {
      ...
      <ul className="flex flex-wrap justify-center gap-6 md:gap-10">
        {CATEGORIES.map((item) => {
          ...
          return (
            <li key={item.slug}>
              <Link
                href={item.slug === "all" ? "/experiences" : `/experiences?category=${item.slug}`}
                className="flex flex-col items-center gap-3 text-sm text-text"
              >
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {CATEGORIES.map((item) => {
        const Icon = ICONS[item.slug];
        const href = item.slug === "all" ? "/experiences" : `/experiences?category=${item.slug}`;
